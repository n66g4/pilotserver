...
默认构建版本为 `1.0.23-1`；如需自定义可使用 `PILOTSERVER_SPK_VERSION=版本 ./synology/build-spk.sh`。同一版本重装经常不刷新套件图标，换图标后请升版本或先卸载再装。

## DSM 安装与升级

1. 套件中心 → 设置 → 勾选 **信任未知发布者**（黑群晖必备）
2. 手动安装 → 选中 `dist/pilotserver-1.0.23-1-x64.spk`（不要用旧的 `*-x86_64.spk`）
3. 向导填写 **管理密码**；可选填 **公网 HTTPS 访问地址**（不是配 DDNS）与配对二次口令
4. **DDNS** 仍用 DSM「外部访问 → DDNS」

从旧版本升级时也在套件中心选择“手动安装”，上传 `pilotserver-1.0.23-1-x64.spk` 并按提示升级；升级前仍建议备份 `/var/packages/pilotserver/var/data/`。`1.0.23-1` 将套件图标改为 Tesla 蓝；`1.0.21-1` 起提供统一的概览 / 设备 / 行程 / 设置控制台，支持简体中文与英文切换并持久保存语言选择，并支持在设置中导入已添加到 GitHub 的 SSH 私钥。此次升级不涉及 API 或数据库迁移，原有数据与设备端配置可继续使用。安装或升级完成后确认套件状态为“运行中”，再打开 `/admin/`。升级后若显示的指纹不是你的 GitHub 私钥指纹，请先清除，再重新粘贴保存。
