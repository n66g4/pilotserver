...
自建 OpenPilot / DragonPilot 云端服务：设备配对、Athena 在线、外网 SSH 隧道、行程上传、OTA 元数据、管理后台。  
设备端**不安装额外二进制**，只改 fork 默认 `API_HOST` / `ATHENA_HOST`（或 `launch_env.sh`）。
...
| OTA | HTTP 元数据 + 静态产物（Git updater 需另改 fork，见文档） |
...
当前 cereal schema 锁定到 DragonPilot `pre-build` 分支、commit `21d40d72c65021c81e84a62e23d700972c7c8a7f`。完整 `rlog` 信号分析和 Cabana 集成仍属于后续范围。
...
# 默认产物：dist/pilotserver-1.0.20-1-x64.spk
```

`1.0.20-1` 提供统一的概览 / 设备 / 行程 / 设置控制台，可在简体中文与英文间切换并记住语言选择。此次升级不涉及 API 或数据库迁移；原有数据和设备端配置可继续使用。

在管理后台「设置」中生成并复制 NAS 公钥，将公钥加入车上 `comma` 用户的 `authorized_keys` 并启用 SSH；私钥始终只保留在 NAS。随后可在管理后台设备页点击「开终端」。原有可选的复制 SSH 命令方式仍需放行 `41000–41099` 端口。
